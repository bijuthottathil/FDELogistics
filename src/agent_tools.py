...
def get_cached_huggingface_embeddings(model_name: str):
    """
    Loads and locks the HuggingFace model weights into the process's RAM.
    If called again with the same model_name (e.g. on the next request in
    this long-running Chainlit server), the cached instance is returned instantly.
    """
    print(f"🧠 MEMORY SEED: Permanently caching local model [{model_name}] in process RAM...")
...
def search_compliance_sop(query: str) -> str:
    ...
    Use this to retrieve regulatory thresholds, cold-chain breach mitigations, rerouting rules,
    cargo condition code handling (OK/DAMAGED/SPOILED), and route risk index escalation rules.
    """
    try:
