...
def parse_and_chunk_document(doc_path: Path) -> list[Document]:
    ...
    if ext == ".md":
        ...
        pdf_docs = []
        # Numbered SOP sections (e.g. "1. Cargo Condition Monitoring") get split out
        # individually, mirroring the header-aware treatment given to .md files above,
        # instead of dumping a whole page in as a single undifferentiated chunk.
        section_header_pattern = re.compile(r"(?m)^(\d+\.\s+.+)$")
        try:
            with open(doc_path, "rb") as f:
                ...
                for page_num, page in enumerate(reader.pages):
                    ...
                    matches = list(section_header_pattern.finditer(page_text))
                    if not matches:
                    ...
                    for idx, match in enumerate(matches):
                        ...
                        if section_text:
                            pdf_docs.append(Document(
                                ...
                                metadata={"page_number": page_num + 1, "section": section_title},
                            ))
